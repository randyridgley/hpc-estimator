...
gpu_predicate = 'num_gpus > 0'
cpu_predicate = 'num_gpus == 0'
...
cpu_cross_df = cpu_cross_df.filter(col('vcpu') >= col('num_cpus')).filter(col('memory') >= col('mem_gb'))
...
gpu_cross_df = gpu_cross_df.filter(col('vcpu') >= col('num_cpus')).filter(col('memory') >= col('mem_gb'))
...
df_estimate = df_estimate.withColumn("job_cost", expr("cast(round((spotprice * node_ct) * walltime_hrs, 2) as float)")) \
    .drop('rank')
